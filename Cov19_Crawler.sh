...
# 月,日,検査実施人数（日別）,検査実施人数（累計）,陽性患者数（日別）,陽性患者数（累計）
awk 'BEGIN{FS = ","}{Negative_ByDay= $3 - $5; Negative_Cumulative = $4 - $6; if($3 == 0){Positive_Rate = "-"} else {Positive_Rate = $5 / $3 * 100;} print $0","Negative_ByDay","Negative_Cumulative","Positive_Rate;}' | \
awk 'BEGIN{print "月,日,検査実施人数（日別）,検査実施人数（累計）,陽性患者数（日別）,陽性患者数（累計）,陰性患者数（日別）,陰性患者数（累計）,陽性率(the last six digits)";}{print;}' > "陽性率_"$FileNameHash".csv"
echo "陽性率の計算完了"
